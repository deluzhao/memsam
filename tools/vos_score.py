...
color_map = {obj_id: np.random.randint(30, 220, size=(3,)).tolist() for obj_id in range(100)}
...
def vos_inference(
    ...
    for frame_idx in frame_inds:
        try:
            ...
            masks[frame_idx] = per_obj_input_mask
        except FileNotFoundError as e:
            raise RuntimeError(
                f"In {video_name=}, failed to load input mask for frame {frame_idx=}. "
        ...
        if frame_idx in input_frame_inds:
            for object_id, object_mask in per_obj_input_mask.items():
                ...
                if object_id not in object_ids_set:
                    raise RuntimeError(
                        ...
                        f"later {frame_idx=} (but not appearing in the first frame). "
                        "Please add the `--track_object_appearing_later_in_video` flag "
                        "for VOS datasets that don't have all objects to track appearing "
                        "in the first frame (such as LVOS or YouTube-VOS)."
                    )
...
def main():
    ...
    for n_video, video_name in enumerate(video_names):
        ...
        # if video_name in ['d83wYdy0', 'x3nD3QQ9']:
