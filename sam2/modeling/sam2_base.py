...
class SAM2Base(torch.nn.Module):
    ...
    def _score_and_select_memory(
        ...
        t_pos_and_prevs = [(0, out, None) for out in selected_cond_outputs.values()]
        l = len(t_pos_and_prevs)

        # Pre-process frames
        all_frames = [None] * (frame_idx)
        for idx in range(frame_idx):
            all_frames[idx] = output_dict["non_cond_frame_outputs"].get(idx, None)
            if all_frames[idx] is None:
                all_frames[idx] = unselected_cond_outputs.get(idx, None)
        ...
        for i, frame in enumerate(all_frames):
            if frame is None:
                continue
            pred_mask = frame['pred_masks']
            (maxes, _) = torch.max(pred_mask.flatten(1), dim=1)

            score[i] = (i, (maxes > 0).long())
        ...
        t_pos = self.num_maskmem - 1
        obj_count = torch.zeros(n).to(t_pos_and_prevs[0][1]['pred_masks'].device)
        chosen_frames = []
        while torch.sum(obj_count < (self.num_maskmem - 1)) > 0:
            ...
            out = all_frames[prev_frame_idx]

            if out is not None:
                # Check if there are objects that need to be added to to_cat_memory
                collected_objs = (obj_count < (self.num_maskmem - 1)) * score[prev_frame_idx][1]
                if torch.sum(collected_objs) > 0:
                    obj_count += collected_objs
                    t_pos_and_prevs.append((prev_frame_idx, out, collected_objs))
                    # t_pos_and_prevs.append((enc_pos, out, collected_objs))
                    chosen_frames.append(prev_frame_idx)
                    # num_collected += 1
                    # enc_pos -= 1
                # meta.append((prev_frame_idx, t_pos, num_collected, obj_count))


            t_pos -= 1
            
            # if insufficient number of memory frames containing object
            max_frames = torch.max(obj_count)
            for i in range(n):
                j = l
                while obj_count[i] < max_frames:
                    # set empty frame as memory frame for object
                    obj_count[i] += 1 - t_pos_and_prevs[j][2][i]
                    t_pos_and_prevs[j][2][i] = 1
                    j += 1
        ...
        hybrid = True
        interp = "minmax"
        for t, (idx, prev, objs) in enumerate(t_pos_and_prevs):
            if prev is None:
                continue  # skip padding frames

            # "maskmem_features" might have been offloaded to CPU in demo use cases,
            # so we load it back to GPU (it's a no-op if it's already on GPU).
            feats = prev["maskmem_features"].to(device, non_blocking=True).flatten(2).permute(2, 0, 1)
            for i in range(n):
            ...
            if objs is None:
                maskmem_enc = (
                    maskmem_enc + self.maskmem_tpos_enc[self.num_maskmem - idx - 1]
                )
            # non-cond frame
            else:
    ...
    def _encode_new_memory(
        ...
        maskmem_out["vision_features"] = maskmem_features
    ...
    def _encode_memory_in_output(
        ...
        if run_mem_encoder and self.num_maskmem > 0:
            ...
            maskmem_out = self._encode_new_memory(
